...
parser.add_argument(
    ...
    type=str,
    help="The path to a list file where each line is a session (sess-#####) processed in the original BAW predicthd run",
    default="",
    required=False,
)
parser.add_argument(
    "--bad_sessions_list",
    type=str,
    help="The path to a list file where each line is a session (sess-####) that should not be processed",
    default="",
    required=False,
)
...
if args.bad_sessions_list != "":
    with open(args.bad_sessions_list) as fd:
        bad_sessions = fd.read().splitlines()
...
for session in sessions:
    if (
        ...
        if args.bad_sessions_list == "" or Path(session).name not in bad_sessions:
            ...
            if (
                saved_sessions_count >= args.session_count and args.session_count != -1
