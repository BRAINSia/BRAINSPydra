...
class BIDSFilename(object):
    def __init__(self, raw_filename):
        ...
        attribute_list_of_lists = [elem.split("-") for elem in self.__basename.split("_")]   

        self.attribute_dict = { sub_list[0]: sub_list[1] for sub_list in attribute_list_of_lists if len(sub_list) == 2 }
        self.attributes_wo_keys = [sub_list[0].replace(f".{self.extension}", "") for sub_list in attribute_list_of_lists if len(sub_list) == 1]
        self.attribute_order = [sub_list[0] for sub_list in attribute_list_of_lists if len(sub_list) == 2]
    ...
    def get_basename(self):
        ...
        ordered_attr_dict = OrderedDict(sorted(self.attribute_dict.items(), key=lambda x: self.attribute_order.index(x[0])))
        # join the dictionary
        keyed_attr_string = "_".join(["-".join([k, v]) for k, v in ordered_attr_dict.items()])
