...
processing_node = pydra.Workflow(
    name="processing_node", input_spec=["input_image"], input_image=input_image
)
processing_node.split("input_image")

antsApplyTransforms_workflow = pydra.Workflow(
    name="antsApplyTransforms_workflow1",
    input_spec=["input_image", "reference_image", "transforms"],
    input_image=processing_node.lzin.input_image,
)
antsApplyTransforms_workflow.inputs.reference_image = [
    "/mnt/c/2020_Grad_School/Research/output_dir/sub-273625_ses-47445_run-002_T1w/t1_average_BRAINSABC.nii.gz",
    "/mnt/c/2020_Grad_School/Research/output_dir/sub-273625_ses-47445_run-002_T1w/t1_average_BRAINSABC.nii.gz",
]
antsApplyTransforms_workflow.inputs.transforms = [
...
antsApplyTransforms_task.inputs.input_image = (
    antsApplyTransforms_workflow.lzin.input_image
)
...
antsApplyTransforms_task.inputs.transforms = (
    antsApplyTransforms_workflow.lzin.transforms
)
...
antsApplyTransforms_workflow.add(
    get_self(name="get_self", x=antsApplyTransforms_task.inputs.reference_image)
)
