...
def make_cache_dir(input_sub):
    cache_dir = (
        Path("/mnt/c/2020_Grad_School/Research/BRAINSPydra/cache_dir")
        / Path(input_sub).with_suffix("").with_suffix("").name
    )
    # print(cache_dir)
    return cache_dir
...
shelly_workflow = pydra.Workflow(
    name="shelly_workflow",
    input_spec=["input_data"],
...
source_node.add(
    get_input_field(
        name="get_session", input_dict=source_node.lzin.input_data, field="session"
    )
)
shelly_workflow.add(
...
processing_node.set_output(
    [
        ("out", processing_node.shelly_workflow.lzout.all_),
...
sink_node2.add(
    copy(
        name="copy4",
        source_output_dir=sink_node2.lzin.output_directory,
        session=sink_node2.lzin.session,
