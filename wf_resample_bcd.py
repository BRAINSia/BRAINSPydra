...
if __name__ == "__main__":
    ...
    os.system(
        f"rm -rf {cache_dir}/*"
    )  # Only deleting cache now as the pipeline is being developed and tested
    ...
    outer = pydra.Workflow(
        name="outer", input_spec=["t1_list"], t1_list=subject_t1s, cache_dir=cache_dir
    )
    ...
    source_node.add(
        get_subject(name="get_subject", subject_list=source_node.lzin.t1_list)
    )
    ...
    bcd.inputs.outputLandmarksInACPCAlignedSpace = (
        wf.outputLandmarksInACPCAlignedSpace.lzout.out
    )
    ...
    resample.inputs.referenceVolume = (
        wf.BRAINSConstellationDetector.lzout.outputResampledVolume
    )
    ...
    wf.set_output(
        [
            ...
            (
                "writeBranded2DImage",
                wf.BRAINSConstellationDetector.lzout.writeBranded2DImage,
            ),
    ...
    sink_node.inputs.outputLandmarksInACPCAlignedSpace = (
        wf.lzout.outputLandmarksInACPCAlignedSpace
    )
    ...
    sink_node.add(
        copy_from_cache(
            name="outputLandmarksInInputSpace",
            cache_path=sink_node.lzin.outputLandmarksInInputSpace,
    ...
    sink_node.add(
        copy_from_cache(
            name="outputResampledVolume",
            cache_path=sink_node.lzin.outputResampledVolume,
            output_dir=output_dir,
        )
    )
    sink_node.add(
        copy_from_cache(
            name="outputTransform",
            cache_path=sink_node.lzin.outputTransform,
            output_dir=output_dir,
        )
    )
    sink_node.add(
        copy_from_cache(
            name="outputLandmarksInACPCAlignedSpace",
            cache_path=sink_node.lzin.outputLandmarksInACPCAlignedSpace,
            output_dir=output_dir,
        )
    )
    sink_node.add(
        copy_from_cache(
            name="writeBranded2DImage",
            cache_path=sink_node.lzin.writeBranded2DImage,
            output_dir=output_dir,
        )
    )
    sink_node.add(
        copy_from_cache(
            name="outputVolume",
            cache_path=sink_node.lzin.outputVolume,
            output_dir=output_dir,
        )
    )
